...
class RegistryClient extends Base {
  ...
  subscribe(reg, listener) {
    ...
    for (let item of regs)
      this.emit(consumeEventName, item);
  ...
  unSubscribe(reg, listener) {
    ...
    if (reg.type !== TYPE_CONSUMER || reg.pid === undefined)
      return;
...
function generateEventName(reg) {
  if (reg.type === TYPE_CONSUMER && reg.pid !== undefined)
    return `${reg.type}${JOIN_STRING}${reg.pid}`;
  if (reg.type === TYPE_OK || reg.type === TYPE_ERROR)
    return `${reg.type}${JOIN_STRING}${reg.name}`;
