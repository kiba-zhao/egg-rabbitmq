  ...
  onChannelChanged() {
    ...
    if (listeners)
      for (let key in listeners)
        registry.unSubscribe(...listeners[key]);
...
async function prepareQueue(consumer) {
  const channel = await prepareChannel(consumer);
  const { options } = consumer[MODEL_KEY];
  const contains = await channel.checkQueue(options.queue);
  if (contains)
    return;
  await channel.assertQueue(options.queue, options.queneOpts);
  await channel.bindQueue(options.queue, options.exchange, options.pattern, options.args);
}

async function prepareConsume(consumer) {
  const channel = await prepareChannel(consumer);
